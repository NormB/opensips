...
is bounded by the longest single-line value (`X-Request-Id`, 12 chars).
Long paths split across `<br>` breaks to avoid stretching the column.
...
| `persist_path`      | string | `/var/lib/`<br>`opensips/`<br>`nats_consumer/`<br>`handles.json` | Destination path.  Parent directory must exist at init time. |
