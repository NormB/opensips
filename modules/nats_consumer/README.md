...
| `allow_sync_in_request_route` | int | `0`                                          | Opt-in switch that widens the route mask of the sync `nats_request` so it may be called from `REQUEST_ROUTE` / `FAILURE_ROUTE` / `BRANCH_ROUTE` / `ERROR_ROUTE`.  Default 0 keeps the parser rejecting bare `nats_request(...)` from those contexts (since the sync call blocks the worker for up to `timeout_ms`).  Set to 1 only after confirming the worst-case `timeout_ms × concurrent_RPCs / udp_workers` fits the deployment's SIP latency budget.  The recommended pattern from any worker route is `async(nats_request(...), rt)`, which yields the worker on an eventfd and is accepted regardless of this modparam. |
