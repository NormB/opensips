...
> Path convention used in this document: `$STATE_DIR = /var/lib/opensips`.
> The module embeds the resolved absolute path at build time; the
> `$STATE_DIR` shorthand exists only to keep the table readable.
...
| `persist_path`      | string | `$STATE_DIR/nats_consumer/handles.json`       | Destination path.  Parent directory must exist at init time. |
