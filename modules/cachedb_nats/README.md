...
| `kv_history` | int | 1 | Version history depth per key. Keep at 1 for registration buckets; raise only if your script uses `nats_kv_history()`. Expiry is performed by the periodic cleanup scan (`nats_reap_interval`). |
| `kv_ttl` | int | 0 | Bucket-wide TTL in seconds (0 = no expiry); applies to the generic `cache_store`/`cache_add` paths. **Must stay 0 for registration buckets** (a whole-bucket max age would silently delete permanent contacts; the module refuses to start otherwise). Per-key expiry for usrloc rows is handled by the reaper (cleanup scan). |
