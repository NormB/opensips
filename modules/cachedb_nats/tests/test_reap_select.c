...
static int cdbn_reap_row_due(int64_t row_exp, time_t now, int grace)
{
#ifdef REAP_CURRENT
...
static enum reap_action cdbn_reap_row_action(int n_live_survivors)
{
...
int main(void)
{
	...
	CHECK(cdbn_reap_row_due(2000, now, S) == 0, "future row_exp => not due");
	CHECK(cdbn_reap_row_due(0, now, S) == 0, "row_exp==0 (permanent) => NEVER due");
