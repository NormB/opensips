...
int main(void)
{
	...
	EQ(_ttl_classify(TTL_PUB_OK, 0), TTL_DONE, "NATS_OK => DONE");
	EQ(_ttl_classify(TTL_PUB_JS_ERR, 10071), TTL_RETRY, "10071 WrongLastSeq => RETRY");
	EQ(_ttl_classify(TTL_PUB_JS_ERR, 10166), TTL_LATCH_OFF, "10166 TTLDisabled => LATCH_OFF");
	EQ(_ttl_classify(TTL_PUB_JS_ERR, 10165), TTL_ASSERT_BUG, "10165 TTLInvalid => ASSERT_BUG");
	EQ(_ttl_classify(TTL_PUB_CONN_DOWN, 0), TTL_FAIL_SAVE, "TIMEOUT/CLOSED => FAIL_SAVE");

	printf("[TREV-13] OK ignores any stale jerr; unknown JS error fails the save:\n");
	EQ(_ttl_classify(TTL_PUB_OK, 10071), TTL_DONE, "OK + stale jerr => still DONE");
	EQ(_ttl_classify(TTL_PUB_JS_ERR, 99999), TTL_FAIL_SAVE, "unknown JS error => FAIL_SAVE");
	EQ(_ttl_classify(TTL_PUB_JS_ERR, 0), TTL_FAIL_SAVE, "JS error with no code => FAIL_SAVE");
	EQ(_ttl_classify(TTL_PUB_CONN_DOWN, 10071), TTL_FAIL_SAVE, "conn down ignores jerr => FAIL_SAVE");
