 ...
 *
 * _reap_project_survivors(json,len,now,grace,&n_survivors,&out_len):
...
static int _reap_row_due(int64_t row_exp, long now, int grace)
{ return row_exp != 0 && (row_exp + (int64_t)grace) <= (int64_t)now; }
static int _reap_row_due_json(const char *json, int len, long now, int grace)
{
	int64_t row_exp;
	...
	long now, int grace)
{
	...
	while (p < c_ve) {
		...
		if (!p) return -1;
		has = (_contact_expires(cvs, p, &e) == 0);
		...
		if (_sink_putc(s, ':') < 0) return -1;
		if (_sink_write(s, cvs, (int)(p - cvs)) < 0) return -1;
	}
	return _sink_putc(s, '}');
}

static char *_reap_project_survivors(const char *json, int len, long now, int grace,
	int *n_survivors, int *out_len)
{
	...
	{
		const char *q = _skip_ws(c_vs, c_ve);
		...
		while (q < c_ve) {
			...
			if (*q == '}') break;
			if (*q == ',') { q++; continue; }
			q = _parse_json_string(q, c_ve, &name, &nlen);
			...
			q = _skip_ws(q, c_ve);
			cvs = q;
			q = _skip_json_value(q, c_ve);
			if (!q) { free(surv); free(allc); return NULL; }
			has = (_contact_expires(cvs, q, &e) == 0);
...
int main(void)
{
	...
	o = _reap_project_survivors(d, (int)strlen(d), now, S, &n, NULL); }
	...
	o = _reap_project_survivors("{\"foo\":1}", (int)strlen("{\"foo\":1}"), now, S, &n, NULL);
	...
	CHECK(_reap_row_due_json("{\"row_exp\":900}", 15, now, S) == 1, "row_exp 900 +5<=1000 => due (1)");
	CHECK(_reap_row_due_json("{\"row_exp\":996}", 15, now, S) == 0, "row_exp 996 +5>1000 => not due (0)");
	CHECK(_reap_row_due_json("{\"row_exp\":0}", 13, now, S) == 0, "row_exp 0 (permanent) => never due (0)");
	CHECK(_reap_row_due_json("{\"aorhash\":7}", 13, now, S) == -1, "absent row_exp => -1 (legacy, fail-closed project)");
	CHECK(_reap_row_due_json("{\"row_exp\":5000000000}", 22, now, S) == 0, "post-2038 row_exp not due (int64, no clamp)");

	printf("[adversarial] malformed JSON => NULL, no crash/leak:\n");
	CHECK(_reap_project_survivors("[1,2,3]", 7, now, S, &n, NULL) == NULL, "non-object top-level => NULL");
	CHECK(_reap_project_survivors("{\"contacts\":}", 13, now, S, &n, NULL) == NULL, "contacts value not an object => NULL");
	CHECK(_reap_project_survivors("", 0, now, S, &n, NULL) == NULL, "empty => NULL");
	CHECK(_reap_project_survivors(NULL, 5, now, S, &n, NULL) == NULL, "NULL => NULL");
