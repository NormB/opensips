...
fn main() {
    ...
    // Compile and run version probe
    let probe_src = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap())
        .join("sdk/version_probe.c");
    ...
    // Fallback: use generic values (will fail version check but at least compiles)
