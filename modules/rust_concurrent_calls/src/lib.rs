...
//!
//! KNOWN ISSUE — per-worker thread_local MI read bug (deferred, post-summit):
//! UDP workers increment their own thread_local CountMap / limit_overrides while
//! the MI process (PROC_MODULE, rank -2, never serves SIP) reads its own empty
//! copies — so concurrent_show returns {} regardless of real load and
//! concurrent_override writes in one worker are invisible to the others.
//! A shm-backed refactor was attempted (commit 50c346dbe) but reliably SIGSEGVs
//! the OpenSIPS attendant during mod_init on this aarch64 build (rustc 1.94
//! cdylib + opensips_rs_shm_malloc interaction — root cause undiagnosed;
//! same failure class as rust_acl's shm_counters_init). Reverted to the
//! pre-refactor thread_local implementation to restore bootability for the
//! Summit 2026 stage demo. Aggregate tile counters (STAT_CHECKED /
//! STAT_ALLOWED / STAT_BLOCKED / STAT_ACTIVE_CALLS) tick correctly via the
//! shared StatVar subsystem; the per-account CURATED table is the regressed
//! surface. Proper fix: diagnose why shm_malloc-from-mod_init crashes for
//! some Rust cdylibs but not others. Repro: re-apply 50c346dbe, rebuild,
//! `docker compose up opensips` — SIGSEGV in attendant before fork, no
//! `access_policy:` log line, container restart-loops.
